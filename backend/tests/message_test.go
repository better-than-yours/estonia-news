...
func (t *SuiteTest) Test_Message_FormatText() {
	...
	`
	assert.Equal(t.T(), "Ööl vastu 24.", service.FormatText(message1))
...
 
	...
	assert.Equal(t.T(), "View this post on Instagram\nA post shared by Pirinen & Salo (@pirinensalo)\nSoome arhitektuuribüroo Pirinen & Salo disainis Porovesi järve kaldale väga erilise hüti.", service.FormatText(message2))
	...
	Ööl vastu 24.`
	assert.Equal(t.T(), "Ööl vastu 24.", service.FormatText(message3))

	message4 := `Ööl vastu 24.<img src="http://feeds.feedburner.com/~r/delfiuudised/~4/t4DO-Uy3On4" height="1" width="1" alt=""/>`
	assert.Equal(t.T(), "Ööl vastu 24.", service.FormatText(message4))
}
